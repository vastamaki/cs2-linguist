...
fn announce(app: &tauri::AppHandle, runtime: &Runtime) {
    let _ = app.emit("engine-status", &runtime.status);
}
...
pub fn start(app: &tauri::AppHandle) -> Result<(), String> {
    ...
    settings.validate()?;
    for id in [&settings.model[..], "vad"] {
        let model = models::model(id)?;
        if !state.data.join("models").join(model.filename).exists() {
            return Err(
                "Download or import the selected speech model and speech detector first.".into(),
            );
        }
    }
    let mut runtime = state.runtime.lock().unwrap();
    ...
    launch_or_fallback(app, &mut runtime, settings)
}
...
fn launch_or_fallback(
    ...
    let result = launch(app, runtime, &settings);
    if let Err(error) = result {
        if settings.backend == Backend::Gpu {
            ...
            if let Err(error) = launch(app, runtime, &settings) {
                return failed(app, runtime, error);
            }
        } else {
            return failed(app, runtime, error);
        }
...
fn failed(app: &tauri::AppHandle, runtime: &mut Runtime, error: String) -> Result<(), String> {
    runtime.status.running = false;
...
fn launch(
    ...
    let mode = if settings.backend == Backend::Cpu {
        "cpu"
    } else {
        "gpu"
    };
    let filename = format!("linguist-worker-{mode}{}", std::env::consts::EXE_SUFFIX);
    ...
    let message = WorkerCommand::Start {
    ...
    runtime.status.backend = settings.backend;
    runtime.status.phase = "loading".into();
    runtime.status.message = "Starting local speech engine…".into();
    ...
    std::thread::spawn(move || {
        ...
        let error = error_message.unwrap_or_else(|| format!("Speech worker exited unexpectedly ({exit:?}). Check your model files and graphics driver."));
        if settings.backend == Backend::Gpu {
            runtime.status.generation += 1;
            runtime.status.warning = Some(format!("GPU unavailable: {error} Using CPU."));
            settings.backend = Backend::Cpu;
            let _ = launch_or_fallback(&app, &mut runtime, settings);
        } else {
            let _ = failed(&app, &mut runtime, error);
        }
    });
    Ok(())
}
